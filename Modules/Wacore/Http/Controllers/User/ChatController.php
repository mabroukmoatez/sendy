...
class ChatController extends Controller
{
    ...
    public function chatHistory($id)
    {
        ...
        // Cache is causing issues with real-time updates, temporarily remove or shorten
        ...
        //     }
        // );
        $response = $this->getChats($device->id); // Directly call getChats

        if ($response["status"] == 200) {
            $data["chats"] = $response["chats"]; // Ensure this is 'chats'
    ...
    public function sendGroupMessage(Request $request, $id)
    {
     ...
     public function getChatMessages(Request $request, $device_uuid, $jid)
    {
        $device = Device::where("user_id", Auth::id())
            ->where("status", 1)
        ...
        // Call Node.js API to get messages for this JID
        $nodeAppUrl = env('WA_SERVER_URL');
        $session_id = 'device_' . $device->id;

        try {
            // Assuming you'll create a Node.js endpoint like /chats/messages/{jid}?id={sessionId}
            $response = Http::get("{$nodeAppUrl}/chats/messages/{$jid}", [
                'id' => $session_id,
            ]);
            ...
            if ($response->successful()) {
                ...
                if (isset($messagesData['success']) && $messagesData['success']) {
                    ...
                    Log::error('Node.js API Error (Get Chat Messages): ' . json_encode($messagesData));
                    return response()->json([
                        'success' => false,
                        'message' => $messagesData['message'] ?? 'Failed to retrieve messages from Node.js.'
                    ], 500);
                }
            } else {
                Log::error('Failed to fetch messages from Node.js API: ' . $response->status() . ' - ' . $response->body());
                return response()->json([
                    'success' => false,
                    'message' => 'Failed to connect to Node.js API for messages.'
                ], 500);
            }
        } catch (\Exception $e) {
            Log::error('General error fetching chat messages: ' . $e->getMessage());
            return response()->json([
                'success' => false,
