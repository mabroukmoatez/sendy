	...
	<div class="col-12">
		...
		<div class="card">
			<div class="card-header row" style="display: flex; justify-content: space-between; align-items: center;">
				<h4 class="text-left col-6" style="font-size: 20px; margin-bottom: 0;">{{ __('Schedules') }}</h4>
				<ul class="nav nav-pills custom-btn-div">
					<li class="nav-item  ">
						<a href="{{ route('user.schedule-message.create') }}" class="nav-link active btn btn-sm"><i class="ni ni-calendar-grid-58"></i>&nbspCreate Schedule</a>
					</li>
				</ul>
			</div>
			<div class="card-body">
							...
							<tbody class="tbody">
								@foreach($posts as $post)
									<tr>
										...
										<td class="text-center">{{ $post->body !=  null ? 'Texte brut' : 'Template'  }}</td>
