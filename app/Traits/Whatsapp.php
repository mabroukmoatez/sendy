   ...
   private function getChats($device_id)
    {
        ...
        if ($status != 200) {
            ...
            $contacts = $colllections->map(function($item) {
                             // --- MODIFIED: Ensure 'id' is used instead of 'number' ---
                             $data['id'] = $item->id; // Use the original JID as 'id'
                             $data['name'] = $item->name ?? $item->id; // Use name, fallback to JID
                             $data['unreadCount'] = $item->unreadCount ?? 0;
                             $data['conversationTimestamp'] = $item->conversationTimestamp ?? 0;
                             // --- END MODIFIED ---
                             return $data;
                        });

            $responseData['status'] = 200;
            $responseData['chats'] = $contacts; // Ensure the key is 'chats'
        }
